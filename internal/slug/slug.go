...
// label (1-63 chars) with no leading/trailing hyphen and no `--` run. `--` is the
// org/app separator in the content host, so a slug containing it would break the
// org-namespacing invariant (HostForSite's collision-free guarantee assumes
