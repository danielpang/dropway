...
-- `dropway_app` role for authorization, so it needs SELECT.
--
...
-- migrate. dropway_app already has USAGE on the schema (migration 0001); it gets only
-- SELECT here — never write/DDL, which stays with Better Auth.

-- +goose Up
-- +goose StatementBegin
GRANT SELECT ON ALL TABLES IN SCHEMA auth TO dropway_app;
-- +goose StatementEnd
-- +goose StatementBegin
ALTER DEFAULT PRIVILEGES IN SCHEMA auth GRANT SELECT ON TABLES TO dropway_app;
-- +goose StatementEnd

-- +goose Down
-- +goose StatementBegin
ALTER DEFAULT PRIVILEGES IN SCHEMA auth REVOKE SELECT ON TABLES FROM dropway_app;
-- +goose StatementEnd
-- +goose StatementBegin
REVOKE SELECT ON ALL TABLES IN SCHEMA auth FROM dropway_app;
-- +goose StatementEnd
