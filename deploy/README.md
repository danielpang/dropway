...
Company memory is **off** until you set `EMBEDDINGS_API_KEY`. Each org also opts in
under **Settings → Company memory**. Chat-log extraction additionally needs
`OPENROUTER_API_KEY` (bring your own key; you pay the provider directly). Content
indexing of published sites and skills runs from the embeddings key alone. See
`deploy/.env.example` for `EMBEDDINGS_*` and `AI_MEMORY_*`.
