...
type Config struct {
	...
	//
	// The PostHog key + host are NOT held here: the shared posthog-go client (used by
	// both error tracking and product analytics) is built in main() from
	// internal/phclient, which reads POSTHOG_KEY / POSTHOG_HOST directly so there is
	// a single source of truth for the client config.
	Environment string
