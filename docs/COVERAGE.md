...
each component's behavior is actually verified. Measured with the following:
...
> what it *is*. **Pure logic** gets a high unit target. **DB/external-bound code**
> gets a low unit target plus full coverage by the docker integration suite
> (`-tags integration` / `-tags 'cloud integration'`). **Wiring/generated code** is
> excluded (built and integration-exercised, never unit-tested).
...
| `internal/auth` | ≥ 90% | **81.0%** | residual is JWKS-fetch error branches (integration-exercised); iss/aud enforcement now guarded in `config` |
...
| `cloud/billing` | ≥ 80% | **61.4%** | webhook dedupe/apply, price→tier, signature parse, owner/admin gate, store logic are all unit-tested; the **real Stripe network client** is integration/manual, not faked |
...
its target), for example parse `go test -cover ./...` and compare against the targets
