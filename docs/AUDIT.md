...
# Code audit — findings & disposition

A full audit of the Phases 0–4 codebase (Go + TypeScript) for bugs, dead code,
silent failures, and security/tenant-isolation regressions, run alongside the
coverage push (`docs/COVERAGE.md`). Each finding's status: **FIXED**,
**ACCEPTED** (intentional / low risk, kept as-is), or **DEFERRED** (tracked
follow-up).
...
|---|---|---|---|
| **HIGH** | auth | `services/api/internal/config/config.go` | golang-jwt **skips iss/aud validation when the expected value is empty**, so an empty `JWT_ISSUER`/`JWT_AUDIENCE` would accept *any* EdDSA-signed unexpired token. → `Load()` now **fails fast** when `JWKS_URL` is set but either is empty (`config_validate_test.go`). |
| **MEDIUM** | dead code | `services/api/internal/handlers/sites.go` | Duplicate `case errors.Is(err, store.ErrHostTaken)` in `writeStoreError` — the second was unreachable. → removed. |
| **MEDIUM** | test quality | `apps/dashboard/lib/audit.ts` | `isSecurityAction` silently **failed to highlight the real `site.access_change`** action (the `\b` after "access" doesn't match `_change`); the test only asserted fabricated strings. → widened the pattern and added `test/audit-canonical.test.ts` asserting the **canonical Go action vocabulary** (mirrors `internal/audit`). |
| **LOW** | store RLS | `services/api/internal/store/authz.go` | `ResolveForPassword` set `app.current_user_id` to the **org id** (password mode is anonymous). Benign now, latent mis-scoping trap. → set the user GUC empty. |
