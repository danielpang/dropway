...
> *A folder of files becomes a live, access-controlled URL in one command. Static now, dynamic-ready. Built on Cloudflare (R2 + Workers), a Next.js control plane, a Go deploy/runtime backend, and Supabase Postgres.*

This plan was produced after reading Shopify's "Quick" writeup (https://shopify.engineering/quick) to copy its proven feature set, then extending it for true multi-tenancy and 3-tier sharing.
...
Shopify's internal **Quick** proved a sharp thesis: *sharing things is harder than building them.* Quick collapsed "a folder of files" into "a live secure URL" with one command, with no pipeline and no config. But Quick is **single-tenant, internal, single-access-model**: every site is open to every Shopify employee, with no per-site permissions or ownership.
...
- **Three sharing tiers per site:** (a) explicit email allowlist, (b) anyone in the org, (c) public (listed, unlisted-signed, or password-protected).
...
| `quick deploy` CLI wrapping rsync | **ADAPT** | Keep one command; rsync becomes R2 presigned multipart + content-addressed manifest, per-user auth, org-scoped. |
| Overwrite-on-redeploy (mutate in place) | **ADAPT** | Replace with immutable, atomic, content-addressed deploys + pointer flip, giving versioning/rollback. |
...
| Google IAP (all-employees) | **ADAPT** | Replace with a thin edge Worker + **Go-API authz** (and a gated-site `/authz` exchange over Better Auth) for external users + 3 tiers, not one Google org. |
| Single access model, no per-site perms | **DROP** | The product *is* per-site ownership + 3 tiers. Direct divergence. |
| CloudSQL + Go API server | **ADAPT** | CloudSQL becomes Supabase Postgres; keep Go for deploy orchestration + runtime. |
...
| Rate limiting | **KEEP** | Essential externally for denial-of-wallet on public/runtime. |
...
- **Serving:** Dispatch Worker on `*.dropwaycontent.com`; resolve host to org/site/deploy via KV; auth; stream from R2 **[MVP]**; Cache API for public only, never cache private **[MVP]**; per-site security headers **[MVP]**; custom domains via Cloudflare for SaaS **[Enterprise]**.
- **Runtime (architected now, built later):** Identity API, rate limiting, collection-DB CRUD **[v1]**; realtime, file uploads, LLM/image proxy **[v1/later]**; WebSockets/DOs, SSR via Workers-for-Platforms **[later]**. Going from static to dynamic adds a user-Worker behind the same Host; **no rewrite**.
- **Billing & limits (cloud-only):** Member-count bands with **hard caps**: **Free ≤5 members & ≤10 sites/user, Business 6 to 99, Enterprise 100 to 1,000, Contact Sales >1,000** **[MVP]**. Exceeding a cap **blocks the action and opens the Stripe subscription modal** (or sales CTA at the top). Stripe, one Customer per org; **the paid `plan_tier` is persisted to the DB by a signature-verified webhook** (not the browser redirect); seats from Better Auth `member` rows; metered egress + runtime **[plans MVP, metering v1]**. **All limits/billing run only in the hosted-cloud build; self-host is unlimited (see §14).**
- **Open source + SaaS (open-core)** **[MVP]:** Repo is public under **FSL-1.1-Apache-2.0** (self-host free, no resale; converts to Apache 2.0 in 2 yrs); a `cloud/` module (quotas + Stripe + subscription modal) and an `ee/` module (enterprise features) are separately licensed and excluded from the self-host build. One-command self-host (Docker Compose / Helm). Details in §14.
...
- **Scale/perf:** Match/beat 50k sites cheaply; architect for ≥1M. Cache-hit < 20ms from PoP; KV routing 0.5 to 10ms. Respect platform limits (KV 1 write/sec/key, so never counters; D1 caps; replica lag).
- **Cost:** Exploit R2 free egress; cache aggressively to cut Class-B ops; per-tenant cost attribution + denial-of-wallet caps.
- **Observability:** Structured logs (deploys, edge auth decisions, `/authz` mints); metrics (requests/site, cache-hit ratio, R2 ops, auth failures, deploy latency); `request_id` correlated across edge, `/authz`, Go, and Postgres.
...
**Request lifecycle, public site (the 95% path):** browser to PoP to Serving Worker, which resolves `route:<host>` from KV and streams the version's bytes from R2 (Cache API). **No JWT, no broker, fully cacheable.**

**Request lifecycle, gated site [Phase 2]:** `password` mode: the Worker prompts, verifies, and sets a host-scoped signed cookie (no identity needed). `allowlist`/`org-only` mode: with no valid host-scoped cookie, a 302 goes to `app.dropway.dev/authz?return=<host>`, Better Auth confirms identity and checks allowlist/membership, then issues a **short-lived, host-scoped** signed token (cookie on the *site* host, or a one-time `?token=` the Worker exchanges). The Worker verifies **that host-scoped token** (its own secret/JWKS), *never* the operator dashboard JWT. This cross-domain exchange is the genuinely hard piece and is deliberately deferred out of Phase 0/1.
...
**Recommended: Next.js (App Router).** It is constraint-aligned and hosts Better Auth (`/api/auth/*`) and the gated-site `/authz` exchange in one BFF.
...
**Recommended (chosen): Vercel for the dashboard** (`app.dropway.dev`: Next.js + Better Auth + the `/authz` exchange). **Cloudflare keeps the whole content plane: R2 object store, the serving Worker on `*.dropwaycontent.com`, and KV/D1.** This split is natural here, because the dashboard and content already live on different domains, and the heavy, cost-sensitive traffic (served tenant sites, free R2 egress) stays on Cloudflare while only the light management UI runs on Vercel. The **public serve path never touches Vercel or carries a JWT**; only the gated-site `/authz` exchange (Phase 2) does. Self-host swaps Vercel for a container (the dashboard is just a Next.js app).
...
| Fiber | Fast | Built on fasthttp, which breaks the `net/http` ecosystem; long-term liability |
...
| Option | Pros | Cons |
|---|---|---|
| **Better Auth** ✅ | TS-native, self-hosted, owns tables in *our* Postgres; **Organization plugin = orgs/members/roles/invitations/access-control out of the box** (covers reqs 3 and 4); **JWT plugin + JWKS** preserves edge verify; Google/email/magic + email-verification + SSO/SAML plugins; no per-MAU vendor bill | Younger than incumbents; we run/upgrade it; org access-control rules are ours to define & test |
...
| Cloudflare Access | Managed SSO at the network edge | Can't gate the Vercel-hosted dashboard; not for end-user per-object authz, so **not used here** |
| Signed-URL only | Simple; great for bulk public download | URL secrecy is not a boundary; can't model tiers/revoke |

**Recommended: keep the public serve path JWT-free** (Worker = thin router: KV host→version → R2, cacheable, the 95% case). **Gated tiers [Phase 2] use a host-scoped token exchange** at `app.dropway.dev/authz` (the Worker verifies that token, not the operator JWT, per §6). **The real authz boundary is the Go API origin** (it verifies every JWT). **Better Auth login + a platform-staff role** gate the dashboard and internal admin (no Cloudflare Access); signed URLs narrowly for upload/bulk download.
...
**Recommended: Go = sqlc + pgx/v5 over the `app` schema (migrations via `goose`); Better Auth OWNS + migrates its own identity/org tables (in an `identity` schema); the dashboard consumes a generated OpenAPI TS client, NOT Postgres (no Drizzle, no `supabase-js`).** The only genuine cross-language *data* contract is the Worker's KV value shape, defined once in `contracts/` (JSON Schema → Go struct + TS type + round-trip test). There is no single migration engine over the union: Better Auth and goose each own their schema, with a documented, CI-gated migration order. Full rationale in §8.
...
- **Policies are simple and subquery-free:** `USING (org_id = current_setting('app.current_org_id', true)::uuid)`, with no joins/helper-function lookups on the hot path. RLS is the isolation *backstop*; the **Go API is the primary authz layer**.
...
- **Authorization split (reqs 3 and 4), enforced in the Go API + a DB CHECK/trigger:** toggling `org_meta.allow_external_sharing` and writing `member.role` requires admin/owner; per-site sharing is writable by the site owner OR an org admin, but a CHECK/trigger **rejects `public`/external grants when the org policy is false**.
- **Quota race safety (hard caps):** `count(*)`-then-insert races. Instead, within the create tx, **`SELECT … FOR UPDATE` the org's `org_usage` row**, check the cap, increment, then insert. This serializes per-org creates without global locks. Domain claims rely on the `hostname` unique constraint. **Downgrades that exceed the new cap go to a read-only-over-limit state, never destructive deletion.**
...
- **Tools: reads direct, WRITES through the API.** Read tools (`list_sites`, `list_files`, `read_file`, `download_site`) run straight off the RLS-scoped store. WRITE tools (`create_site`, `set_site_access`, `deploy_site`) do **not** mutate Postgres or the projection directly; they call the **Go API** over HTTP, forwarding the user's OAuth token, so every write reuses the API's quota, **admin re-check** (set_site_access is owner/admin only, mirroring the dashboard), **edge-route projection**, revocation, and audit. `deploy_site` runs the full deploy loop through the API (prepare → presigned blob upload → finalize → publish), computing the byte-identical content digest via the shared `internal/manifest`. The API keeps the §6 invariant that it is the *only* projection writer. To accept the forwarded token the API adds the MCP resource to its verifier's accepted audiences (`MCP_PUBLIC_URL`); the token already carries `org_id`+`sub`, and admin-gating re-checks the live `identity.member` from those, so no extra claims are needed. The write tools are registered only when the MCP server has an `API_URL` (else it stays read-only).

**Revocation story:** the common case is just the **short token/JWT TTL** (5 to 15 min), minted from the revocable Better Auth DB session, so killing the session stops re-mint. **Hard revocation** (ban, org removal) adds a **KV denylist / per-subject `min_iat`** checked at the authz exchange, built in **Phase 4**, not speculatively.

**Edge projection pipeline (connective tissue, Phase 1+):** the **Go API** writes the KV/D1 projection on publish (`route:<host>` + access_mode + allowlist) and is the **only writer**, with the Worker reading only. Postgres is authoritative and the projection is **fully rebuildable** from it (carry a `schema_version`; a reconciler re-pushes on drift; DR drill in §13). When an admin flips `allow_external_sharing`→false or unshares, the Go API rewrites the affected routes within the propagation window.
...
**Rule: each table has exactly one writer-owner; every other runtime consumes a *generated* artifact, never a hand-written duplicate.** There is no single migration engine over the union, because that fought Better Auth. Instead:
...
|---|---|---|---|---|
| **Members / org (HARD CAP)** | **≤ 5** | **6 to 99** (cap 99) | **100 to 1,000** (cap 1,000) | **> 1,000** (no self-serve) |
...
| Egress overage | n/a | $0.20/GB | $0.15/GB | negotiated |
...
**The hard caps & their gates (req 4 + new bands):** checked **synchronously in the Go control plane (`cloud/quota`)** at the cost-creating action, namely `POST /sites` (count the user's owned sites) and member-add/invite-accept (count the org's members), read against the org's **live `subscriptions.plan_tier`**:
...
- `customer.subscription.deleted`: downgrade to **Free**: `plan_tier='free'`, `status='canceled'`; if the org now exceeds Free caps, set `org_status='over_limit'` (read-only/no-new-members + banner), but **never delete data**.
...
- **Phase 3: Cloud billing & quotas (proprietary, cloud-only).** Stripe subscriptions + webhook lifecycle (**entitlement persisted by signed webhook**, §9); map tiers to the Phase-0 `QuotaProvider`; transactional **`org_usage` FOR UPDATE** hard-cap enforcement (Free 5u/10s → Business <100 → Enterprise 100 to 1,000 → Contact Sales); over-limit/downgrade → **read-only, not destructive**; 402 → subscription modal. OSS core still runs with the no-op provider.
...
- **Core repo → `FSL-1.1-Apache-2.0`** (Functional Source License, Sentry's). Grants **everything except "Competing Use"**: you may self-host, modify, and use Dropway internally (including at a for-profit company), but you **may not offer Dropway (or a derivative) to third parties as a commercial/hosted service**. Each release **auto-converts to Apache 2.0 two years later**, so the project still becomes truly open over time. This is the practical, adoption-friendly reading of "self-host but can't make money off it."
  - *Note on terminology:* FSL is **source-available**, not an OSI-"Open Source" license, since by definition no OSI license can forbid commercial/SaaS use. If a real OSI badge is required later, the fallback is **AGPLv3** (allows competition but forces source-sharing of networked forks); we deliberately chose FSL over AGPL to actually block resale.
...
For reference, so the backlog is read against what exists. Phase 4 delivered, with tests, and kept all Phase 0 to 3 tests green:
