...
- The MCP OAuth provider flow rides on the same Better Auth sessions — the
  consent route carries its own enforcement gate (a server layout above the
  client page), so an enforced-but-unenrolled member cannot mint MCP/CLI
  tokens; an enrolled one passes through unchanged.
- **Known enforcement boundary (follow-up)**: the gates run on server-rendered
  page loads (the app layout, the consent layout, the setup page). App Router
  soft navigations within an already-open tab and server actions do not pass
  through layouts, so a member who was signed in when enforcement flipped on
  can keep using that tab until their next full page load. Every privileged
  write still re-checks role/membership in the Go API. The deeper fix is a
  shared per-request seam (Next middleware calling a policy endpoint, or the
  session helper treating an unenrolled session under require_mfa as
  not-fully-authenticated) — deliberately not bolted on here.
